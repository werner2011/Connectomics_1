...
def analyze_geodesic_distance(neuron: navis.TreeNeuron):
    """
    Oblicza geodezyjną odległość każdego węzła
    od korzenia neuronu.
    weight="weight" oznacza wykorzystanie
    rzeczywistej długości krawędzi.
    """
...
def analyze_sholl(neuron: navis.TreeNeuron,n_radii=30):
    ...
    Liczy przecięcia arboru z kolejnymi
    sferami wokół somy lub korzenia.
    """
    if neuron.has_soma:
        center = "soma"
...
def summarize_morphology(neuron,segments, geodesic, sholl):
    """
    Tworzy jeden wiersz podsumowania
    morfologicznego dla neuronu.
    """
    tortuosity = (
        segments["tortuosity"]
        .replace(
    ...
    return {
        ...
        "geodesic_median": geodesic["distance_to_root"].median(),
...
def plot_distribution( df,column,title,xlabel):
    """
    Histogram rozkładu wybranej cechy.
    """
    values = (
        df[column]
        .replace(
...
def main() -> None:
    ...
    for neuron in neurony:
        ...
        row = summarize_morphology( neuron,segments,geodesic,sholl)
        summary.append(row)
